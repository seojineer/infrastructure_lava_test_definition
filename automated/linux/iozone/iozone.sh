...
usage() {
    echo "Usage: $0 [-s <skip_install>] [-v <version>] [-o <options> option '-a', '-I' are already included.]" 1>&2
    exit 1
}

while getopts "s:v:o:c:h" opt; do
    case "$opt" in
        s) SKIP_INSTALL="${OPTARG}" ;;
        v) VERSION="${OPTARG}" ;;
        o) OPTIONS="${OPTARG}" ;;
        c) CHECK_COMPLETE="${OPTARG}" ;;
        *) usage ;;
    esac
done
...
iozone -a -I $OPTIONS | tee "$LOGFILE"

if [ "$CHECK_COMPLETE" = 'true' ] || [ "$CHECK_COMPLETE" = 'True' ]; then
    if [ -n "$(sed -n '/iozone test complete/p' $LOGFILE)" ]; then
        echo "iozone-test-complete pass" | tee -a "$RESULT_FILE"
    else
        echo "iozone-test-complete fail" | tee -a "$RESULT_FILE"
    fi
else
    # Parse iozone stdout.
    field_number=3
